...
async function main() {
    let endpointUrl = "opc.tcp://172.21.3.1:4840";
    let conveyorType = "2";
    let machineName = "ASRS";
    ...
    while (true) {
        ...
        if (IsConnectToBMES) {
            ...
            ts = tnow.toISOString();
            console.log(`\n--> Submit Transaction: Checkin to ${machineName} at ${ts}`);

            let checkInResult = await contractProd.submitTransaction('CheckIn', carrierId.toString(), machineName, ts);
        ...
        if (IsOnDuty == "Yes" || !IsConnectToBMES) {
            ...
            if (IsConnectToBMES) {
                tnow = new Date(Date.now());
                ts = tnow.toISOString();
                console.log(`\n--> Submit Transaction: ReportTransitionStart Carrier ${carrierId.toString()} @${machineName} at ${ts}....`);
                result = await contractProd.submitTransaction('ReportTransitionStart', carrierId.toString(), machineName, ts);
            }
